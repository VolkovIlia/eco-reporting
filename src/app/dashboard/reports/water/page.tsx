...
export default async function WaterReportPage({ searchParams }: Props) {
  const { user } = await requireSession();
  const { reportId: reportIdStr } = await searchParams;

  if (!reportIdStr) redirect("/dashboard/reports");
  ...
  if (!report || report.type !== "2tp_water") notFound();
  ...
  return (
    <div className="max-w-4xl space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">2-ТП Водхоз</h1>
        <p className="text-sm text-gray-500 mt-1">
          Форма федерального статистического наблюдения № 2-ТП (водхоз)
        </p>
      </div>
      <Card>
        <WaterForm
